...
def _row_to_budget_entry(row) -> BudgetEntry:
    ...
    for col, field in optional_map.items():
        val = r.get(col)
        if val is not None:
            # Dates are stored as Date columns; the dict API stays ISO strings
            # (calculations parse entry["date"] with date.fromisoformat).
            entry[field] = val.isoformat() if col == "date" else val
...
def update_budget_entry(
    ...
    for k, v in updates.items():
        col = _field_to_col(k)
        if col:
            merged[col] = to_date(v) if col == "date" else v
