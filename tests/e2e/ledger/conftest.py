...
def _create_ledger_snapshot(base_url: str) -> None:
    """All ledger pages live under /s/<snapshot>; tests use one named ledger."""
    _form_post(f"{base_url}/files/new", {"name": "ledger"})
...
def flask_server(tmp_path_factory):
    """Start Flask on a random port with an otherwise-empty temp database.

    A "ledger" snapshot is created so the /s/ledger/* pages resolve.
...
def confirmed_server(tmp_path_factory):
    ...
    html = _form_post(
        f"{base_url}/s/ledger/import/accounts",
        {
    ...
    _form_post(f"{base_url}/s/ledger/import/{import_id}/confirm")
